...
回归还包含直接读取结构化ABI的 `audit_test`：18个用户态场景检查游标边界、多读取者、
精确覆盖计数、多次环绕、统计不变量与无自反馈；另已验证同一次QEMU启动中连续运行两遍。
实现与验证记录见 [游标、覆盖与统计测试设计](../../docs/AUDIT_TEST_DESIGN.md#11-用户态实现与实际验证2026-09-05)。
独立主机生命周期场景仍是后续计划，本目录的主机用例数量保持28项。
