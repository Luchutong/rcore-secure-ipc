...
pub fn main() -> i32 {
    let results = [
        ...
        check(
            "write null",
            unsafe { write(1, core::slice::from_raw_parts(bad_ptr(0), 16)) },
        ),
        // 2. write: 内核物理地址 0x80200000
        check(
            "write kernel-addr",
            unsafe { write(1, core::slice::from_raw_parts(bad_ptr(0x8020_0000), 16)) },
        ),
        // 3. write: 顶端越界地址
        check(
            "write high-addr",
        ...
        check(
            "pipe kernel-addr",
            unsafe { pipe(core::slice::from_raw_parts_mut(0x8020_0000 as *mut usize, 2)) },
        ),
        // 8. read: 向内核地址写入
        check(
            "read kernel-addr",
            unsafe { read(0, core::slice::from_raw_parts_mut(bad_mut_ptr(0x8020_0000), 16)) },
        ),
    ];
