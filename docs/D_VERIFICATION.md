...
结果：宿主侧 28/28 通过；QEMU 25 个正常用例与 4 个预期失败用例，共 29/29 通过。`quota_test`、`ipc_audit_integration_test`、`auditctl_test` 和 `audit_test` 均通过。GitHub Actions `34331641132` 的文档构建和 QEMU 用户测试也均成功，功能分支的发布任务按预期跳过。
