...
> **完成状态（2026-09-14）**：本计划所列 A/B/C/D 四模块已经完成正式集成，
> `integration` 通过完整验收后由最终 PR 合入 `main`。测试、性能复测与发布证据见
> [最终验收与发布记录](FINAL_ACCEPTANCE.md)。
...
- [x] 冻结 `integration`；
- [x] 运行完整 CI 和验收清单；
- [x] 通过最终 PR 将 `integration` 合入 `main`；
- [x] 创建版本标签并保存测试日志。

## 6. 合并验收清单

- [x] 原版 rCore 用户测试全部通过；
- [x] Rust 文档可以构建，CI 不使用过时的只读令牌发布文档；
- [x] 未授权信号被拒绝，返回值符合 API v1；
- [x] 无效用户地址不会导致内核 panic；
- [x] 文件描述符和管道资源可回收，无退出泄漏；
- [x] 每类 IPC 操作都有成功和失败审计记录；
- [x] 至少包含正常、越权、边界和并发四类测试；
- [x] 有原版与改造版的定量性能对比；
- [x] 四名成员的提交、测试和文档责任可由 Git 历史核查。
