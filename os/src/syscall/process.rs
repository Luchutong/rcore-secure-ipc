...
use crate::mm::{
    MAX_ARGV, MAX_STR_LEN, try_translated_ref, try_translated_refmut, try_translated_str,
};
use crate::task::{
...
pub fn sys_exec(path: *const u8, mut args: *const usize) -> isize {
    ...
    loop {
        ...
        let Some(arg_str_ptr) = try_translated_ref(token, args) else {
            return -1;
        };
        let arg_str_ptr = *arg_str_ptr;
        if arg_str_ptr == 0 {
            break;
        }
...
pub fn sys_sigaction(
    ...
    if let Some(flag) = SignalFlags::from_bits(1 << signum) {
        ...
        // 校验两个用户指针：非法地址返回 -1，不 panic
        let Some(old_action_slot) = try_translated_refmut(token, old_action) else {
            return -1;
        };
        let Some(action_ref) = try_translated_ref(token, action) else {
            return -1;
        };
        *old_action_slot = prev_action;
        inner.signal_actions.table[signum as usize] = *action_ref;
        0
    } else {
        -1
    }
